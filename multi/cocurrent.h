...
class safe_ptr
{
	...
	friend class std::lock_guard; // MSVC 2013
...
public:
	contention_free_shared_mutex() : shared_locks_array_ptr(std::make_shared<array_slock_t>()), shared_locks_array(*shared_locks_array_ptr), want_x_lock(false), recursive_xlock_count(0),
...
template <typename T>
using contfree_safe_ptr = safe_ptr<T, contention_free_shared_mutex<>,
